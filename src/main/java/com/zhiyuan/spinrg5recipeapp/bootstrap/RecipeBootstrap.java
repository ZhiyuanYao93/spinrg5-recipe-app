...
public class RecipeBootstrap implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent contextRefreshedEvent) {
        ...
        log.debug("Loading bootstrap data for H2 DB ");
    }
    ...
    //get UOM optionals
        ...
        Optional<Category> usCatOptional = categoryRepository.findByDescription("American");

        if (!usCatOptional.isPresent()){
            throw new RuntimeException("American category not found");
        }

        Optional<Category> mexicoCatOptional = categoryRepository.findByDescription("Mexican");

        if (!mexicoCatOptional.isPresent()){
            throw new RuntimeException("Mexican category not found");
        }

        Optional<Category> italyCatOptional = categoryRepository.findByDescription("Italian");

        if (!italyCatOptional.isPresent()){
        ...
        Category usCategory = usCatOptional.get();
        Category mexicoCategory = mexicoCatOptional.get();
        Category italyCategory = italyCatOptional.get();
        Category fastfoodCategory = fastfoodCatOptional.get();
        ...
        guacaRecipe.setDirections("1 Cut avocado, remove flesh: Cut the avocados in half. Remove seed. Score the inside of the avocado with a blunt knife and scoop out the flesh with a spoon" +
                "\n" +
        ...
        guacaRecipe.getCategories().add(usCategory);
        guacaRecipe.getCategories().add(mexicoCategory);

        guacaRecipe.setUrl("https://goo.gl/jbC9i8");

        recipeList.add(guacaRecipe);
        ...
        tacosRecipe.getCategories().add(usCategory);
        tacosRecipe.getCategories().add(mexicoCategory);

        tacosRecipe.setUrl("https://goo.gl/nPJ1so");
